...
def test_update_exercise_name_only():
    # Setup
    setup = client.post("/prs", json={"exercise": "Typo Name", "weight": 50, "reps": 10})
    pr_id = setup.json()["id"]
...
def test_delete_pr():
    setup_response = client.post("/prs", json={"exercise": "Run", "weight": 10, "reps": 1})
